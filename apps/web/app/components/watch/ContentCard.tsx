 ...
 * Page stays visible behind modal — builds desire, improves conversion.
 */
...
export function ContentCard({ item, variant = 'portrait', showInfo = true }: ContentCardProps) {
  ...
  const handleClick = () => {
    // Guest → show modal (page stays visible behind it)
    if (!isAuthenticated) {
      setShowAuthModal(true);
      return;
    }

    // Free user + premium content → upgrade
    if (item.isPremium && !userIsPremium) {
      router.push('/choose-plans');
      return;
    }

    // Authenticated and allowed → go to player
  ...
  return (
    <>
      ...
      {/* Auth prompt modal — renders over the page, not a redirect */}
      <AuthPromptModal
