 ...
 *   1. PremiumCard now links to /watch/[slug] ÃƒÂ¯Ã‚Â¿Ã‚Â½ full player experience
 *   2. Single click on card navigates to watch page (primary behaviour)
 *   3. Inline overlay player kept as fallback ÃƒÂ¯Ã‚Â¿Ã‚Â½ triggered via a secondary
...
function PremiumCard({ item, isLoading, onWatchPage, onPlayInline }: {
  ...
  return (
    <div className="premium-card" style={{ position: 'relative', cursor: 'pointer' }}>
      ...
      {/* Secondary "Play here" button ÃƒÂ¯Ã‚Â¿Ã‚Â½ inline overlay fallback */}
      <button
...
function VideoPlayer({ url, mimeType, onClose, onError }: {
  ...
  const handleError = (e: React.SyntheticEvent<HTMLVideoElement | HTMLAudioElement>) => {
    ...
    onError(msgs[code] || 'Playback error ÃƒÂ¯Ã‚Â¿Ã‚Â½ please try again.');
  };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#000',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none',
          color: '#fff', fontSize: 28, cursor: 'pointer', lineHeight: 1 }}>ÃƒÂ¯Ã‚Â¿Ã‚Â½</button>
      </div>
...
export default function PremiumAccessPage() {
  ...
  // Premium check ÃƒÂ¯Ã‚Â¿Ã‚Â½ tolerant of both 'ACTIVE' and 'active'
  ...
  const loadContent = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        // SubscriptionGuard blocking ÃƒÂ¯Ã‚Â¿Ã‚Â½ fall back to fetching all and filtering
  ...
  const handlePlayInline = async (item: ContentItem) => {
    ...
    try {
      ...
      if (res.ok) {
        const d = await res.json();
        // Controller returns playUrl ÃƒÂ¯Ã‚Â¿Ã‚Â½ also accept url/originalUrl as fallbacks
        const mediaUrl = d.playUrl || d.url || d.originalUrl || '';
        if (mediaUrl) {
          setPlayUrl(mediaUrl);
          // mimeType now returned from controller ÃƒÂ¯Ã‚Â¿Ã‚Â½ critical for correct playback
