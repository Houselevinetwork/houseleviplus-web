    ...
    fun loadHome() {
        viewModelScope.launch {
            ...
            try {
                ...
                // FIX: TravelApi returns Response<List<TravelPackage>> — no .data on List
                val travel: List<TravelPackage> = travelDeferred.await().body() ?: emptyList()

                // FIX: LinearTvNowResponse.current resolves both "nowPlaying" and "block" keys
                val live: LinearTvBlock? = liveDeferred.await().body()?.current
                ...
                _state.value = _state.value.copy(isLoading = false, error = "Failed to load. Check your connection.")
            }
        }
    }
