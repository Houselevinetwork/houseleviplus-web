// src/pages/ReelFilms.tsx
import { useState, useEffect } from "react";
import { Plus, Grid3x3, List, Search, Loader2, RefreshCw, Clapperboard } from "lucide-react";
...
const ReelFilms = () => {
  ...
  const fetchContent = async (showLoading = true) => {
    try {
      ...
      console.log('📥 Fetching reel films...');
      const response = await apiClient.getContent(1, 100);

      if (response.success && response.data) {
        const reelFilmsContent: Content[] = response.data.data
          .filter((item: any) => item.type === 'reelfilm')
          .map((item: any) => ({
            _id: item._id,
            id: item._id,
            title: item.title,
        ...
        setContent(reelFilmsContent);
        console.log(`✅ Loaded ${reelFilmsContent.length} reel films`);
      } else {
        console.error('❌ Failed to fetch content:', response.error);
        toast({
          title: 'Error',
          description: response.error || 'Failed to load reel films',
          variant: 'destructive',
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Failed to load reel films',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this reel film?')) return;
    ...
    try {
      ...
      if (response.success) {
        toast({
          title: 'Success',
          description: 'Reel film deleted successfully',
        });
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to delete reel film',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleEdit = (content: Content) => {
    console.log('Edit reel film:', content);
    toast({
      title: 'Coming soon',
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Manage your reel film collection
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button 
                ...
                className="bg-orange-500 hover:bg-orange-600 transition-all duration-300 shadow-lg"
              >
            ...
            {filteredContent.length > 0 ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-muted-foreground text-lg mb-4">
                  {searchQuery ? 'No reel films found matching your search' : 'No reel films uploaded yet'}
                </p>
                {!searchQuery && (
                  <Button onClick={() => setUploadModalOpen(true)} className="bg-orange-500 hover:bg-orange-600">
                    <Plus className="mr-2 h-4 w-4" />
                    Upload Your First Reel Film
                  </Button>
                )}
              </div>
            )}
