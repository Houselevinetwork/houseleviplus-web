// src/pages/MiniSeries.tsx
import { useState, useEffect } from "react";
import { Plus, Grid3x3, List, Search, Loader2, RefreshCw, Video } from "lucide-react";
...
const MiniSeries = () => {
  ...
  const fetchContent = async (showLoading = true) => {
    try {
      ...
      console.log('📥 Fetching miniseries content...');
      const response = await apiClient.getContent(1, 100);

      if (response.success && response.data) {
        // Filter only miniseries content
        const miniseriesContent: Content[] = response.data.data
          .filter((item: any) => item.type === 'minisode')
          .map((item: any) => ({
            _id: item._id,
            id: item._id,
            title: item.title,
        ...
        setContent(miniseriesContent);
        console.log(`✅ Loaded ${miniseriesContent.length} miniseries episodes`);
      } else {
        console.error('❌ Failed to fetch content:', response.error);
        toast({
          title: 'Error',
          description: response.error || 'Failed to load miniseries',
          variant: 'destructive',
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Failed to load miniseries',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const groupedSeries = (): SeriesGroup[] => {
    ...
    content.forEach((item) => {
      const seriesTitle = (item as any).series?.title || 'Standalone Episodes';
  ...
  const handleEdit = (content: Content) => {
    console.log('Edit episode:', content);
    toast({
      title: 'Coming soon',
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Manage miniseries episodes and seasons
                </p>
              </div>
            </div>
            ...
            {content.length > 0 ? (
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {seriesGroup.totalSeasons} Season{seriesGroup.totalSeasons !== 1 ? 's' : ''} · {seriesGroup.totalEpisodes} Episode{seriesGroup.totalEpisodes !== 1 ? 's' : ''}
                          </p>
                        </div>
                      </div>
              ...
              <div className="text-center py-16">
                ...
                <p className="text-muted-foreground text-lg mb-4">
                  {searchQuery ? 'No miniseries found matching your search' : 'No miniseries uploaded yet'}
                </p>
                {!searchQuery && (
