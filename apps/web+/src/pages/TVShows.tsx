...
import { Plus, Grid3x3, List, Search, Loader2, RefreshCw, Tv } from "lucide-react";
  ...
  const fetchContent = async (showLoading = true) => {
    try {
      ...
      if (response.success && response.data) {
        const tvShowsContent: Content[] = response.data.data
          .filter((item: any) => item.type === 'tv_episode')
          .map((item: any) => ({
            _id: item._id,
            id: item._id,
            title: item.title,
        ...
        setContent(tvShowsContent);
        console.log(`✅ Loaded ${tvShowsContent.length} TV show episodes`);
      } else {
        console.error('❌ Failed to fetch content:', response.error);
        toast({
          title: 'Error',
          description: response.error || 'Failed to load TV shows',
          variant: 'destructive',
        });
      }
    } catch (error) {
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Manage TV show episodes and seasons
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button 
                ...
                className="bg-blue-500 hover:bg-blue-600 transition-all duration-300 shadow-lg"
              >
            ...
            {content.length > 0 ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-muted-foreground text-lg mb-4">
                  {searchQuery ? 'No TV shows found matching your search' : 'No TV shows uploaded yet'}
                </p>
                {!searchQuery && (
                  <Button onClick={() => setUploadModalOpen(true)} className="bg-blue-500 hover:bg-blue-600">
                    <Plus className="mr-2 h-4 w-4" />
