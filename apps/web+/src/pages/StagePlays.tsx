// src/pages/StagePlays.tsx
import { useState, useEffect } from "react";
import { Plus, Grid3x3, List, Search, Loader2, RefreshCw, Theater } from "lucide-react";
...
const StagePlays = () => {
  ...
  const fetchContent = async (showLoading = true) => {
    try {
      ...
      console.log('📥 Fetching stage plays...');
      const response = await apiClient.getContent(1, 100);

      if (response.success && response.data) {
        const stagePlaysContent: Content[] = response.data.data
          .filter((item: any) => item.type === 'stage_play')
          .map((item: any) => ({
            _id: item._id,
            id: item._id,
            title: item.title,
        ...
        setContent(stagePlaysContent);
        console.log(`✅ Loaded ${stagePlaysContent.length} stage plays`);
      } else {
        console.error('❌ Failed to fetch content:', response.error);
        toast({
          title: 'Error',
          description: response.error || 'Failed to load stage plays',
          variant: 'destructive',
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Failed to load stage plays',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this stage play?')) return;
    ...
    try {
      ...
      if (response.success) {
        toast({
          title: 'Success',
          description: 'Stage play deleted successfully',
        });
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to delete stage play',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleEdit = (content: Content) => {
    console.log('Edit stage play:', content);
    toast({
      title: 'Coming soon',
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-lg bg-pink-500/10 flex items-center justify-center">
                <Theater className="h-6 w-6 text-pink-500" />
              </div>
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Manage your stage play collection
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button 
                ...
                className="bg-pink-500 hover:bg-pink-600 transition-all duration-300 shadow-lg"
              >
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search stage plays..."
              value={searchQuery}
          ...
          <>
            {filteredContent.length > 0 ? (
              ...
              <div className="text-center py-16">
                <Theater className="h-16 w-16 text-muted-foreground mx-auto mb-4" />
                <p className="text-muted-foreground text-lg mb-4">
                  {searchQuery ? 'No stage plays found matching your search' : 'No stage plays uploaded yet'}
                </p>
                {!searchQuery && (
                  <Button onClick={() => setUploadModalOpen(true)} className="bg-pink-500 hover:bg-pink-600">
                    <Plus className="mr-2 h-4 w-4" />
                    Upload Your First Stage Play
                  </Button>
                )}
              </div>
            )}
